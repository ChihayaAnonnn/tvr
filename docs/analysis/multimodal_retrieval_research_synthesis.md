...
P2 可从稳定的 log-overlap baseline 出发，但必须重新定义有界 log-variance、`ε` 稳定项、正负 pair 监督和 selective retrieval 目标；同时验证新 uncertainty 不是 WTI score/margin 的单调复制。若不能改善至少一种校准或风险指标，只有 Recall 波动，不应继续堆叠解释性 loss。
...
EagleNet 的 FRL 直接产生推理 logits，但 EAM 只通过训练改变参数；若要声称能量本身改善某一对的风险判断，需要把能量接入 final score 并单独评价。GARE 的 `Δ` 同时进入训练和推理，机制闭环最清楚，但其 relaxed IB 只在训练使用且不是预测 uncertainty。TempMe 的 token 路径两阶段一致，因果解释也最干净。GraviAlign 把 `A+B+C` 称为 ranking score，却未给可复现推理数据流，是当前最大的闭环缺口。

对本项目，任何新增 uncertainty/alignment 分支都必须回答三个问题：训练时监督什么、推理时改变哪个决策、关闭后 final logits 是否严格回到基线。只改变辅助 loss 而不进入排序或风险决策，不能作为核心创新。
...
部署结论不是“pair-conditioned 一定不可用”，而是必须把它放在正确阶段：双塔负责召回，pair scorer 负责有限候选重排；同时报告 k、候选 recall 上限、额外延迟、吞吐、显存和收益。TempMe 则可独立作用于第一阶段，但精度变量与压缩变量不能混在一个消融里。
...
- 新 uncertainty 必须是 query-video pair-level，进入 final logits、重排 gating 或 selective retrieval 风险决策。
- 新机制一次只改变一个因果变量，至少三 seed，报告 T2V/V2T、均值方差和失败条件。
- SOTA 不是唯一成功标准；新颖问题、因果闭环、风险校准、效率边界和可复现性具有同等或更高优先级。

| UATVR 采纳矩阵 | 目标问题 | 机制 | 证据等级 | 采用/拒绝/延后 | Roadmap 阶段 | 前置证据 | 停止条件 | 综合分析锚点 |
|---|---|---|---|---|---|---|---|---|
| EagleNet | 固定 query 对上下文近邻区分不足 | 候选文本采样 + frame/text RGAT | 协议不兼容、部分已核验 | 延后，不照搬全库图 | P3 | P1 证明 top-k 内存在帧上下文型系统错配 | 三 seed 无稳定收益，或 top-k 开销不可接受即停 | `2.2`、`2.6`、`6.2` |
| GARE | modality gap 下 pair-specific 局部修正 | 有界 `Δ_ij` 候选残差/reranker | 协议不兼容、部分已核验 | 延后，仅考虑 top-k | P3 | P1 给出固定 WTI 错配及候选召回上限 | 收益仅来自容量、残差不稳定或 k-cost 曲线无优势即停 | `3.2`、`3.6`、`6.5` |
...
| GraviAlign | pair ambiguity 与距离—uncertainty 耦合 | 有界高斯 log-overlap + 显式 pair 风险头 | 弱证据、实现不可复算 | 只采用 B+C 启发，拒绝原样照搬 | P2 | P1 风险标签/诊断；稳定公式与校准协议 | uncertainty 是 score/margin 单调复制，或不改善风险指标即停 | `5.7`、`5.8`、`6.6` |

### 7.2 延后机制

EagleNet/GARE 的共同洞见是“同一 query 对不同候选需要不同证据”，但二者都把完整库变成 pair computation。只有 P1 证据显示固定 WTI 的错误集中在 top-k 且可由候选上下文纠正，P3 才解锁。首版应比较最简单的 bounded residual 或 risk-gated reranker，不能直接叠加随机采样、图网络、能量模型和多项正则。
...
- 只加辅助 loss、却不说明它如何改变 final ranking/risk decision。
...
1. **Pair-level mismatch uncertainty**：定义 `u(t_i,v_j)` 预测 top-1 错误、pair mismatch 还是相关性噪声；正配对与 high-similarity negative 都要参与监督。评价除 Recall 外至少含 risk-coverage/AURC、top-1 correctness calibration、Brier 或 NLL，并验证 `u` 不是 WTI score/margin 的单调变换。
2. **Uncertainty-gated alignment**：基础 WTI 保持可用；只有高风险候选调用 bounded residual/reranker。需要比较固定 k 与自适应 k，报告 candidate recall ceiling、额外延迟、吞吐、显存和 gate 错误。
...
6. **机制优先的成功标准**：主张必须对应可观测量和失败条件。即使 Recall 没在全部数据集领先，只要风险校准、错误子集、效率 Pareto 或理论—实现一致性形成新证据，也可构成有效科研贡献。
