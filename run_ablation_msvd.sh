...
# Use the best finetuned model (Epoch 1)
# Allow overriding from env; fallback to default finetuned model
MODEL_PATH="${MODEL_PATH:-ckpts/ckpt_msvd_20251223_153240/pytorch_model.bin.0}"

# Log naming: logs/YYYYMMDD/HHMMSS_ablation_msvd.log
RUN_DATE="${RUN_DATE:-$(date +%Y%m%d)}"
RUN_TIME="${RUN_TIME:-$(date +%H%M%S)}"
LOG_DIR="logs/${RUN_DATE}"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_TIME}_ablation_msvd.log}"

echo "=== Starting MSVD Inference-time Ablation ===" | tee -a ${LOG_FILE}
echo "Model: ${MODEL_PATH}" | tee -a ${LOG_FILE}
echo "DSL: True (Enabled for all runs)" | tee -a ${LOG_FILE}
echo "---------------------------------------------" | tee -a ${LOG_FILE}
