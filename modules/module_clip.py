...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = self.positional_embedding[:, None, :].to(x.dtype)
        # pool1 = nn.AdaptiveAvgPool2d((x[0], 1, x[2]))
        # x = x + pool1(y[0], y[1], y[2])
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask=None):
        ...
        self.attn = nn.MultiheadAttention(d_model, n_head)      # rewrite it? check 
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        scale = width ** -0.5   # 768 ** -0.5 = 0.03608..
        self.class_embedding = nn.Parameter(scale * torch.randn(width))     # (768)
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))     # 197 (224*224/16*16), 768
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):     # bs * 12, 3, 224, 224
        ...
        if self.linear_patch == '3d':
            ...
            x = self.conv1(x)  # shape = [*, width, grid, grid]     *, 768, 14, 14
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD    frames * (tokens+1) * Dimension
        ...
        # x = self.ln_post(x) @ self.proj         # frames * (tokens+1) * 768->512
        # x = x[:, 0, :]          # BS*T, 512 -----> embedding for each frame
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(        # modified by BoF
                input_resolution=image_resolution,
    ...
    @staticmethod
    def get_config(pretrained_clip_name):
        # model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "RN50.pt")
        ...
        if pretrained_clip_name in ["ViT-B/32", "ViT-B/16", "ViT-L/14", "ViT-L/14-336"] and os.path.exists(model_path):
            pass
        else:
            if pretrained_clip_name in _MODELS:
                # model_path = _download(_MODELS[pretrained_clip_name])
                model_path = download_model(pretrained_clip_name)
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype) # [batch_size, d_mdoel]
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # NLD -> LND
