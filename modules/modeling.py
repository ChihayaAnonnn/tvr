...
from modules.until_module import AllGather, CrossEn, KLdivergence, MILNCELoss, MILNCELoss_BoF, PreTrainedModel
...
class UATVR(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if self.loose_type is False:
            ...
            cross_config = update_attr(
                "cross_config", cross_config, "num_hidden_layers", self.task_config, "cross_num_hidden_layers"
            )
    ...
    def forward_eval(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        sim_matrix, temp_logits = self.get_similarity_logits(
            sequence_output,
            text_token,
            visual_output,
            attention_mask,
    ...
    def _loose_similarity(
        ...
        if sim_header == "meanP":
            ...
            visual_output = pack_padded_sequence(
                visual_output, torch.sum(video_mask, dim=-1).cpu(), batch_first=True, enforce_sorted=False
            )
        ...
        if self.training:
            ...
            prob_sim_matrix_from_v = torch.einsum(
                "ad,bd->ab", [prob_video_embedding.view(-1, dim), prob_text_embedding.view(-1, dim)]
            )
            MIL_loss_v = self.loss_MIL_fct(prob_sim_matrix_from_v, bs, n_video, n_text)

            prob_sim_matrix_from_t = torch.einsum(
                "ad,bd->ab", [prob_text_embedding.view(-1, dim), prob_video_embedding.view(-1, dim)]
            )  # 与.t()等价
