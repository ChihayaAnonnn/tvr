...
# A3-fixed: soft_bce 概率损失(新默认) + sigmoid 软边界 logvar + warmup 反转,
# RSPR_ANCHOR_WEIGHT=1e-2 为修订版规格的标定起点,梯度诊断全程开启,
# 用日志中的 grad_{prob,rank,anchor}_logvar 验证/修正 λ_a。
RUN_ID="rspr_a3fix_seed0"
...
if [[ -f "${OUTPUT_DIR}/final_test.json" ]]; then
    log "A3-fixed completed successfully: ${OUTPUT_DIR}/final_test.json"
    exit 0
fi
...
log "A3-fixed stopped without final_test.json; inspect ${LATEST_LOG:-no training log found}"
exit 1
