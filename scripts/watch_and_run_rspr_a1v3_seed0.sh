...
# A1-v3: A3fix-v3 的确定性消融——RSPR_MODE=mean、SAMPLE_COUNT=1,其余逐项相同。
# 为什么必须跑:A3fix-v3(随机采样)在 test 上是 T2V 47.0 / V2T 47.4,而旧的 A1
# (mean)是 47.0 / 47.5——打平。但旧 A1 用的是 λx1,和 A3fix-v3 的 λx3 不可比,
# 所以"采样到底有没有用"目前是无法回答的。本 run 补上匹配的 λx3 mean 消融。
# 若 A1-v3 仍与 A3fix-v3 打平,则 §6.5 的随机排序主张在 R@1 上没有证据,
# 只剩 MeanR(A3fix-v3 12.4/9.2 vs 旧 A1 14.2/11.3)可用。
RUN_ID="rspr_a1v3_seed0"
...
if [[ -f "${OUTPUT_DIR}/final_test.json" ]]; then
    log "A1-v3 completed successfully: ${OUTPUT_DIR}/final_test.json"
    exit 0
fi
...
log "A1-v3 stopped without final_test.json; inspect ${LATEST_LOG:-no training log found}"
exit 1
